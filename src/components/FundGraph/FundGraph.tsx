...
export const FundGraph = ({ graphElements }: FundGraphInterface) => {
  ...
  useEffect(() => {
    const updateGraph = async () => {
      ...
      const drag = (simulation: d3.Simulation<FundNodeInterface, SimulationLinkDatum<FundNodeInterface>>) => {
        function dragStarted(event: D3DragEvent<SVGCircleElement, FundNodeInterface, FundNodeInterface>) {
      ...
      const node = svg
        ...
        .join(
          (enter) => {
            ...
            // Card Background
            cardGroup.append('rect').classed('fund-label-card', true).attr('width', '180px').attr('height', '120px').attr('rx', '20px');
            ...
            cardGroup
              ...
              .text((d) => (d.type === 'FUND' && d.fund ? d.fund.name : d.type));
            cardGroup
              .append('text')
            ...
            cardGroup
              ...
              .text((d) => {
                if (d.type === 'FUND' && d.fund) return d.fund.isOpen ? 'Open' : 'Closed';
                return '';
              });
